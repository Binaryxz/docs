"""
Filtra bases brutas de CNPJ por CNAE + capital social/porte + palavras-chave
(config.py), identifica o decisor provável de cada empresa e gera
leads_com_socios.csv — tudo localmente, sem precisar de BigQuery/basedosdados.
...
IMPORTANTE — schema diferente do usado em filtro_cnae.sql/socios_decisor.sql:
este script lê os arquivos ORIGINAIS da Receita Federal, cujo layout de
colunas (nomes abaixo em CAMPOS_*, códigos com zero à esquerda em
situacao_cadastral) é diferente do dataset `basedosdados.br_me_cnpj` usado
pelas queries SQL deste pipeline (que renomeia colunas e remove o zero à
esquerda). Não misture os dois: os nomes de coluna abaixo estão corretos
para os arquivos brutos da Receita, não para basedosdados.

Uso:
    cp config.example.py config.py   # e ajuste pro ICP deste cliente
...
Regra de filtro (igual à de filtro_cnae.sql, mais abrangente):
  ...
  CNAES_ALVO.LIMIAR_PERCENTUAL dos CNAEs alvo (principal + secundários), cujo
  capital social esteja dentro de CAPITAL_SOCIAL_MAX/PORTES_ALVO, OU cuja
  razão social ou nome fantasia contenha alguma palavra-chave de
  PALAVRAS_CHAVE_NOME — tudo configurável em config.py (veja config.example.py).
...
try:
    ...
    raise SystemExit(
        "config.py não encontrado. Copie config.example.py para config.py "
        "e ajuste os valores para o ICP deste cliente antes de rodar:\n"
...
COLUNAS_SAIDA = [
    "cnpj", "razao_social", "nome_fantasia", "capital_social", "porte_empresa",
    "percentual_match_cnae", "qtd_cnaes_alvo_encontrados", "motivo_match",
    "cnae_fiscal_principal", "cnae_fiscal_secundaria", "data_inicio_atividade",
    "uf", "municipio", "ddd_1", "telefone_1", "ddd_2", "telefone_2",
    "correio_eletronico", "nome_decisor", "documento_decisor",
    "qualificacao_socio", "eh_decisor", "data_entrada_sociedade",
]
...
def contem_palavra_chave(texto: str) -> bool:
    if not PALAVRAS_CHAVE_NOME:
        return False
    t = normaliza(texto)
    return any(p in t for p in PALAVRAS_CHAVE_NOME)
...
def capital_social_dentro_do_limite(bruto: str) -> bool:
    if CAPITAL_SOCIAL_MAX is None:
        return True
    valor = (bruto or "").strip().replace(",", ".")
    if not valor:
        return True  # sem dado de capital social, não descarta por isso
    try:
        return float(valor) <= CAPITAL_SOCIAL_MAX
    except ValueError:
        return True


def porte_dentro_do_alvo(porte: str) -> bool:
    if not PORTES_ALVO:
        return True
    return (porte or "").strip() in PORTES_ALVO
...
def escanear_estabelecimentos(
    ...
    for caminho in caminhos:
        for row in abrir_linhas(caminho, CAMPOS_ESTABELECIMENTOS):
            if row.get("situacao_cadastral") != "02":  # 02 = ATIVA (layout original da Receita)
                continue
...
def escanear_empresas(caminhos: list[str], accepted_estab_ids: set[str]) -> tuple[dict[str, dict], set[str]]:
    ...
    for caminho in caminhos:
        for row in abrir_linhas(caminho, CAMPOS_EMPRESAS):
            ...
            if not cnpj_basico:
                continue
            if not capital_social_dentro_do_limite(row.get("capital_social", "")):
                continue
            if not porte_dentro_do_alvo(row.get("porte_empresa", "")):
                continue

            razao = row.get("razao_social", "")
            match_razao = contem_palavra_chave(razao)
            if cnpj_basico in accepted_estab_ids or match_razao:
                empresas_map[cnpj_basico] = {
                    "razao_social": razao,
    ...
    return empresas_map, novos_via_razao_social
...
def main() -> None:
    ...
    parser.add_argument("--limiar", type=float, default=LIMIAR_PERCENTUAL, help=f"Percentual mínimo de CNAEs alvo (padrão: LIMIAR_PERCENTUAL de config.py = {LIMIAR_PERCENTUAL})")
    ...
    print(f"[2/4] escaneando {len(caminhos_emp)} arquivo(s) de empresas (capital social/porte + razão social + join)...", file=sys.stderr)
    empresas_map, novos = escanear_empresas(caminhos_emp, accepted)
    if novos:
    ...
    leads = {cnpj_basico: dados for cnpj_basico, dados in estab_data.items() if cnpj_basico in empresas_map}
    ignorados = accepted - set(leads)
    if ignorados:
        print(f"[aviso] {len(ignorados)} CNPJs aceitos não passaram no filtro de capital social/porte ou não tinham registro em Empresas — ignorados", file=sys.stderr)
    ...
    with open(args.saida, "w", newline="", encoding="utf-8") as f_out:
        ...
        for cnpj_basico, estab in sorted(leads.items(), key=lambda kv: -kv[1]["percentual_match_cnae"]):
            ...
            empresa = empresas_map.get(cnpj_basico, {})
            writer.writerow(
                {
